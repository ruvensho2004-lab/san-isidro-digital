...
export default function Emprendimientos() {
  ...
  return (
    <div>
      <div style={s.header}>
        <h2 style={s.titulo}>💡 Emprendimientos Sociales</h2>
        <button style={s.btnPrimary} onClick={() => setMostrarForm(!mostrarForm)}>+ Nuevo Emprendimiento</button>
      </div>
        ...
        <div style={s.panel}>
          ...
          <div style={s.formRow}>
            <div style={s.formGroup}>
              ...
              <input style={s.input} name="nombre" value={form.nombre} onChange={handleChange} placeholder="Nombre del emprendimiento" />
            </div>
            <div style={s.formGroup}>
              <label style={s.label}>Responsable *</label>
              <input style={s.input} name="responsable" value={form.responsable} onChange={handleChange} placeholder="Nombre del responsable" />
            </div>
          </div>
          <div style={s.formRow}>
            <div style={s.formGroup}>
          ...
          <div style={s.formGroup}>
            ...
            <textarea style={{ ...s.input, minHeight: 90, resize: 'vertical' }} name="descripcion" value={form.descripcion} onChange={handleChange} placeholder="Describe el impacto social..." />
          </div>
      ...
      <div style={s.panel}>
        {loading ? <div style={s.empty}>Cargando...</div> : emprendimientos.length === 0 ? (
          <div style={s.empty}>Sin emprendimientos registrados</div>
        ) : (
          <table style={s.table}>
            <thead><tr>{['#', 'Emprendimiento', 'Responsable', 'Categoría', 'Inversión', 'Estado', 'Acciones'].map(h => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
            <tbody>
              {emprendimientos.map((e, i) => (
                <tr key={e.id}>
                  ...
                  <td style={s.td}>${e.inversion || 0}</td>
                  <td style={s.td}>
